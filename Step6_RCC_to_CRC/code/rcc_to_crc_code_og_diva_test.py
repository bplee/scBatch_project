...
if __name__ == "__main__":
    ...
    if args.train_patient is not None:
        ...
        model_name = f"{args.outpath}_testing_og_diva_data_{args.test_patient}_semi_sup_seed_{args.seed}"
    ...
    print(f"making original data_loader things")
    test_patient = 5
    train_patient = None
    new_train_loader_sup = RccDatasetSemi(test_patient, train_patient=train_patient,
                 train=True, x_dim=784, convolutions=args.conv)
    new_train_loader_unsup = RccDatasetSemi(test_patient, train_patient=train_patient,
    ...
    # setup the VAE
    l = len(data_loaders['sup'].dataset[0][1])
