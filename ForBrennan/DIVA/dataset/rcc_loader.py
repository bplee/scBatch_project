...
class RccDataset(data_utils.Dataset):
    ...
    def _get_data(self):
        ...
        rawdata_tam = readRDS('/data/leslie/alireza/ccRCC/pat6_tams_rawcounts.rds')
        rawdata_tam = rawdata_tam.transpose()
        rawdata_tcell = readRDS('/data/leslie/alireza/ccRCC/pat6_tcells_rawcounts.rds')
        rawdata_tcell = rawdata_tcell.transpose()
        ...
        batch_train[batch_train > self.test_patient] = batch_train[batch_train > self.test_patient] - 1
        batch_test = batch_indices[idx_batch_test].ravel()
        batch_test = np.zeros(len(batch_test)).astype(int)
