...
#from paper_experiments.rotated_mnist.dataset.rcc_loader_semi_sup import RccDatasetSemi
...
def plot_tsne(train_loader, test_loader, model, batch_size, test_patient, cell_types, patients):
    ...
    with torch.no_grad():
        ...
        print('labels_d', labels_d[0:200])
        print('labels_y', labels_y[0:200])
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zy_by_labels_heldout_pat_'+str(test_patient)+'.pdf')

        plt.figure(figsize = (20,14))
        colors = plt.cm.get_cmap('tab10')(np.linspace(0, 1, 10))
        for i, batch in zip(range(len(patients_train)), patients_train):
            print('patients_train:', patients_train)
            print('i:', i)
            print('batch:', batch)
            if i < 10:
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zy_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zd_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zd_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zx_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train_zx_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for (xs, ys, ds) in test_loader:
            i = i + 1
            # To device
            xs, ys= xs.to(device), np.array(ys)
            ...
            zd_.append(np.array(zd_loc.cpu()))
            zx_.append(np.array(zx_loc.cpu()))
            actuals_d.append((5*np.ones(batch_size)).astype(int))
            actuals_y.append(np.argmax(ys,axis=1))
            if i == 50:
               break
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zy_tsne[labels_y == i, 0], zy_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_test_zy_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zd_tsne[labels_y == i, 0], zd_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_test_zd_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zx_tsne[labels_y == i, 0], zx_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_test_zx_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zy_tsne[labels_y == i, 0], zy_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zy_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, batch in zip(range(len(patients)), patients):
            ...
            if i < 10:
               ...
               plt.scatter(zy_tsne[labels_d == i, 0], zy_tsne[labels_d == i, 1], c = colors[i%10], label = batch, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zy_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zd_tsne[labels_y == i, 0], zd_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zd_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, batch in zip(range(len(patients)), patients):
            if i < 10:
               ...
               plt.scatter(zd_tsne[labels_d == i, 0], zd_tsne[labels_d == i, 1], c = colors[i%10], label = batch, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zd_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, cell_type in zip(range(len(cell_types)), cell_types):
            if i < 10:
               ...
               plt.scatter(zx_tsne[labels_y == i, 0], zx_tsne[labels_y == i, 1], c = colors[i%10], label = cell_type, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zx_by_labels_heldout_pat_'+str(test_patient)+'.pdf')
        ...
        for i, batch in zip(range(len(patients)), patients):
            if i < 10:
               ...
               plt.scatter(zx_tsne[labels_d == i, 0], zx_tsne[labels_d == i, 1], c = colors[i%10], label = batch, marker='x')
        plt.legend()
        plt.savefig('./figs_diva/fig_diva_tsne_sup_train+test_zx_by_batches_heldout_pat_'+str(test_patient)+'.pdf')
...
if __name__ == "__main__":
    ...
    for test_patient in range(6):
        ...
        model_name = './../' + 'rcc_test_domain_' + str(test_patient) + '_sup_only_seed_' + str(seed)
        ...
        # Load test
        if supervised:
           my_dataset_train = RccDataset(args.test_patient, args.x_dim, train=True)
           my_dataset_test = RccDataset(args.test_patient, args.x_dim, train=False)
